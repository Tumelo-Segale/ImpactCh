...
function openApproveModal(subId) {
  ...
  if (existing) {
    showConfirm(
      ...
      `Invoice ${existing.invoiceNum} was already issued. View it in the Invoices tab.`,
      () => showPage("invoices"),
      "Go to Invoices"
    );
    return;
  ...
  document.getElementById("approve-sub-title").textContent =
    sub.title.length > 60 ? sub.title.slice(0, 60) + "…" : sub.title;
  document.getElementById("approve-sub-author").textContent =
    sub.submittedByEmail || sub.authors || "";
...
function doApproveAndInvoice(subId, amount) {
  ...
  const updated = subs.map((s) =>
    s.id === subId
      ? {
          ...
          approvedBy: _currentAdmin ? _currentAdmin.name : "Admin",
        }
      : s
  );
  saveSubs(updated);
  ...
  const inv = {
    ...
    ref,
    payLink: ps.payLink || "",
    payLabel: ps.payLabel || "Pay Online",
  ...
  setTimeout(() => {
    ...
    const issueDate = new Date(inv.issuedAt).toLocaleDateString("en-ZA", {
      day: "2-digit",
      month: "long",
      year: "numeric",
    });
    const dueDate = new Date(
      new Date(inv.issuedAt).getTime() + 14 * 86400000
    ...
    const emailSubject = encodeURIComponent(
      "Invoice " + invoiceNum + " - Publication Fee | Impact Compass Holdings"
    );
    const emailBody = encodeURIComponent(
      "Dear " +
        (inv.authors || "Author") +
        ",\r\n\r\n" +
        "Congratulations! Your manuscript has been approved for publication in " +
        (getJournalAbbr(inv.journal) || inv.journal) +
        ".\r\n\r\n" +
        ...
        "\u26a0 IMPORTANT: Payment must be received within 14 days (" +
        dueDate +
        "). " +
    ...
    if (toEmail)
      window.open(
        "mailto:" + toEmail + "?subject=" + emailSubject + "&body=" + emailBody
      );
    showConfirm(
      ...
      toEmail
        ? "Invoice " +
            invoiceNum +
            " created. An email draft has been opened in your mail client. Download the PDF invoice to attach before sending."
        : "Invoice " +
            invoiceNum +
            " for R " +
            amount.toFixed(2) +
            " created. No author email on file - download the PDF to send manually.",
...
function renderInvoicesTable() {
  ...
  if (!invs.length) {
    tbody.innerHTML =
      '<tr><td colspan="6" style="text-align:center;color:var(--text-muted);padding:2rem;">No invoices yet. Approve a manuscript to generate the first invoice.</td></tr>';
    return;
  }
  tbody.innerHTML = invs
    .slice()
    .reverse()
    .map(
      (inv) => `<tr>
        <td style="font-family:monospace;font-size:0.75rem;color:var(--accent);">${escHtml(
          inv.invoiceNum
        )}</td>
        <td><div class="td-title" style="max-width:180px;">${escHtml(
          inv.title.length > 40 ? inv.title.slice(0, 40) + "…" : inv.title
        )}</div></td>
        <td style="font-size:0.78rem;color:var(--text-muted);">${escHtml(
          inv.authorEmail || "-"
        )}</td>
        <td style="font-weight:700;color:#fff;">R ${Number(
          inv.amount || 0
        ).toFixed(2)}</td>
        <td style="font-size:0.75rem;color:var(--text-muted);">${fmtDate(
          inv.issuedAt
        )}</td>
        <td style="display:flex;gap:0.4rem;flex-wrap:wrap;">
            <button class="btn-sm btn-accent" style="padding:0.35rem 0.875rem;" onclick="downloadInvoicePDF('${
              inv.id
            }')">PDF</button>
            <button class="btn-sm btn-ghost"  style="padding:0.35rem 0.75rem;font-size:8px;" onclick="viewInvoiceModal('${
              inv.id
            }')">View</button>
        </td>
    </tr>`
...
function viewInvoiceModal(invId) {
  ...
  document.getElementById("modal-invoice-dl-btn").onclick = () =>
...
function buildInvoiceHTML(inv, forModal) {
  ...
  const logo =
    typeof LOGO_DATA_URI !== "undefined" && LOGO_DATA_URI
      ? `<img src="${LOGO_DATA_URI}" alt="Impact Compass Holdings" style="height:52px;width:auto;display:block;background:transparent;" />`
      : '<strong style="font-size:1.1rem;color:#c19a6b;">IMPACT COMPASS HOLDINGS</strong>';
  return `
<div style="font-family:'Raleway',sans-serif;color:#1a1a1a;max-width:700px;margin:0 auto;padding:${
    forModal ? "0" : "2rem"
  }">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem;">
    <div>
      ...
      <div style="font-size:0.88rem;font-weight:700;color:#111;">${escHtml(
        inv.authors || "Author"
      )}</div>
      <div style="font-size:0.78rem;color:#555;">${escHtml(
        inv.authorEmail || ""
      )}</div>
    </div>
    <div>
      ...
      <div style="font-size:0.78rem;color:#333;line-height:1.5;">${escHtml(
        inv.title.length > 70 ? inv.title.slice(0, 70) + "\u2026" : inv.title
      )}</div>
      <div style="font-size:0.72rem;color:#888;margin-top:2px;">Journal: ${escHtml(
        getJournalAbbr(inv.journal)
      )}</div>
    </div>
  </div>
  <table style="width:100%;border-collapse:collapse;margin-bottom:1.5rem;">
    ...
    <tbody><tr>
      <td style="padding:0.875rem;font-size:0.85rem;color:#222;border-bottom:1px solid #eee;">
        ...
        <span style="font-size:0.72rem;color:#888;">Payment Reference: </span><strong style="color:#c19a6b;">${escHtml(
          ref
        )}</strong>
      </td>
  ...
  <div style="border:1px solid #e8e0d4;padding:1.25rem;margin-bottom:1rem;">
    ...
    ${
      payLink
        ? `<div style="margin-bottom:1rem;"><div style="font-size:0.78rem;font-weight:700;color:#333;margin-bottom:4px;">Online Payment</div><a href="${escHtml(
            payLink
          )}" style="display:inline-block;background:#c19a6b;color:#fff;padding:0.5rem 1.25rem;font-size:0.8rem;font-weight:700;text-decoration:none;">${escHtml(
            payLabel
          )}</a><div style="font-size:0.72rem;color:#888;margin-top:4px;">${escHtml(
            payLink
          )}</div></div>`
        : ""
    }
    ${
      bankName
        ? `<div><div style="font-size:0.78rem;font-weight:700;color:#333;margin-bottom:6px;">EFT / Bank Transfer</div><table style="font-size:0.78rem;color:#444;border-collapse:collapse;width:100%;"><tr><td style="padding:3px 1.25rem 3px 0;color:#888;width:130px;">Bank:</td><td style="font-weight:600;">${escHtml(
            bankName
          )}</td></tr>${
            accName
              ? `<tr><td style="padding:3px 1.25rem 3px 0;color:#888;">Account Name:</td><td style="font-weight:600;">${escHtml(
                  accName
                )}</td></tr>`
              : ""
          } ${
            accNum
              ? `<tr><td style="padding:3px 1.25rem 3px 0;color:#888;">Account No:</td><td style="font-weight:600;">${escHtml(
                  accNum
                )}</td></tr>`
              : ""
          } ${
            branch
              ? `<tr><td style="padding:3px 1.25rem 3px 0;color:#888;">Branch Code:</td><td style="font-weight:600;">${escHtml(
                  branch
                )}</td></tr>`
              : ""
          } ${
            accType
              ? `<tr><td style="padding:3px 1.25rem 3px 0;color:#888;">Account Type:</td><td style="font-weight:600;">${escHtml(
                  accType
                )}</td></tr>`
              : ""
...
function downloadInvoicePDF(invId) {
  ...
  try {
    doc.addImage(LOGO_DATA_URI, "PNG", M, y + 1, 55, 16, undefined, "FAST");
  } catch {
  ...
  const warnLines = doc.splitTextToSize(
    "Payment must be received within 14 days of this invoice date (" +
      dueDate +
      "). Failure to pay will result in the manuscript being automatically disregarded and deemed invalid.",
    W - M * 2 - 6
  );
  doc.text(warnLines, M + 4, y + 10);
  y += 18;
  doc.setFontSize(7);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(...gold);
  doc.text("BILL TO", M, y);
  doc.text("MANUSCRIPT", W / 2 + 2, y);
  y += 4;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(9);
  doc.setTextColor(...dark);
  ...
  doc.setTextColor(...mid);
  doc.text(
    "Journal: " + getJournalAbbr(inv.journal),
    W / 2 + 2,
    y + titleLines.slice(0, 3).length * 4 + 1
  );
  y += 22;
  ...
  doc.text("AMOUNT (ZAR)", W - M - 3, y + 5, { align: "right" });
  y += 8;
  ...
  doc.setFontSize(9);
  doc.setTextColor(...dark);
  doc.text(amtStr, W - M - 3, y + 6, { align: "right" });
  y += 16;
  doc.line(M, y, W - M, y);
  y += 2;
  doc.setFillColor(247, 244, 240);
  ...
  doc.setFontSize(13);
  doc.setTextColor(...gold);
  doc.text(amtStr, W - M - 3, y + 7, { align: "right" });
  y += 16;
  doc.setFont("helvetica", "bold");
  doc.setFontSize(7);
  doc.setTextColor(...gold);
  ...
  if (bankName) {
    ...
    doc.setFontSize(8);
    doc.setTextColor(...dark);
    doc.text("EFT / Bank Transfer:", M, y);
    y += 5;
    const rows = [
      ["Bank:", bankName],
  ...
  doc.rect(M, boxY, W - M * 2, 10, "F");
  doc.setDrawColor(252, 165, 165);
  doc.setLineWidth(0.3);
  doc.rect(M, boxY, W - M * 2, 10, "S");
  doc.setFont("helvetica", "bold");
  doc.setFontSize(7);
  doc.setTextColor(...red);
  doc.text(
  ...
  doc.text(
    "Use your email address as the payment reference. | Thank you for publishing with Impact Compass Holdings.",
    W / 2,
    284,
