...
function loadSubmissions() {
  try {
    ...
    return subs.map((s) =>
      legacyToNew[s.status] ? { ...s, status: legacyToNew[s.status] } : s
    );
  } catch {
    return [];
  }
...
document
  ...
  .addEventListener("submit", function (e) {
    ...
    showConfirm(
      ...
      async () => {
        let manuscriptFilesData = [],
          suppFilesData = [];
        try {
          manuscriptFilesData = await Promise.all(
            _stagedManuscript.map(readFileAsBase64)
          );
          suppFilesData = await Promise.all(_stagedSupp.map(readFileAsBase64));
        } catch {
          manuscriptFilesData = _stagedManuscript.map((f) => ({
            name: f.name,
            size: f.size,
            type: f.type,
            data: null,
          }));
          suppFilesData = _stagedSupp.map((f) => ({
            name: f.name,
            size: f.size,
            type: f.type,
            data: null,
        ...
        try {
          ...
          newSub.manuscriptFiles = _stagedManuscript.map((f) => ({
            name: f.name,
            size: f.size,
            type: f.type,
            data: null,
          }));
          newSub.suppFiles = _stagedSupp.map((f) => ({
            name: f.name,
            size: f.size,
            type: f.type,
            data: null,
        ...
        showSuccess(
          ...
          `Your manuscript "${newSub.title}" has been received. You will receive an invoice via email upon editorial approval.`
        );
...
function renderDashboard() {
  ...
  const subs = user
    ? allSubs.filter(
        (s) => !s.submittedByEmail || s.submittedByEmail === user.email
      )
    : allSubs;
  ...
  el.innerHTML = `<div class="submissions-list">${subs
    .map((sub, idx) => buildSubmissionCard(sub, idx))
    .join("")}</div>`;
}
...
function buildSubmissionCard(sub, idx) {
  ...
  const fileInfo =
    sub.manuscriptFiles && sub.manuscriptFiles.length > 0
      ? `<div style="font-size:0.75rem;color:var(--text-muted);margin-top:0.4rem;">📎 ${sub.manuscriptFiles
          .map((f) => f.name || f)
          .join(", ")}</div>`
      : "";
  ...
  const invoiceNotice = (() => {
    try {
      ...
      const dueDateStr = dueDate.toLocaleDateString("en-ZA", {
        day: "2-digit",
        month: "long",
        year: "numeric",
      });
      ...
      return `
            <div style="margin-top:0.875rem;border:1px solid ${
              isOverdue ? "rgba(239,68,68,0.35)" : "rgba(193,154,107,0.3)"
            };background:${
        isOverdue ? "rgba(239,68,68,0.06)" : "rgba(193,154,107,0.07)"
      };padding:1rem;">
                <div style="font-size:9px;letter-spacing:0.2em;text-transform:uppercase;font-weight:700;color:${
                  isOverdue ? "#f87171" : "var(--accent)"
                };margin-bottom:0.5rem;">Invoice Issued - ${escapeHtml(
        inv.invoiceNum
      )}</div>
                ...
                <div style="font-size:0.78rem;color:var(--text-dim);margin-bottom:0.75rem;">Payment reference: <strong style="color:#fff;">${escapeHtml(
                  inv.authorEmail || sub.submittedByEmail || ""
                )}</strong></div>
                <div style="font-size:0.78rem;padding:0.6rem 0.875rem;background:${
                  isOverdue ? "rgba(239,68,68,0.1)" : "rgba(251,191,36,0.08)"
                };border-left:3px solid ${
        isOverdue ? "#ef4444" : "#f59e0b"
      };color:${isOverdue ? "#fca5a5" : "#fbbf24"};line-height:1.6;">
                    ${
                      isOverdue
                        ? "<strong>⚠ OVERDUE:</strong> The 14-day payment window has passed. This manuscript is now <strong>disregarded and invalid</strong>."
                        : `<strong>⚠ Payment due by ${escapeHtml(
                            dueDateStr
                          )}.</strong> If payment is not received within 14 days, this manuscript will be <strong>disregarded and invalid</strong>.`
                    }
                </div>
            </div>`;
    } catch {
  ...
  return `<div class="glass-card submission-card">
        <div class="submission-head">
            <div>
                ...
                <div class="submission-authors">Authors: ${escapeHtml(
                  sub.authors
                )}</div>
                ${fileInfo}
            </div>
...
function sendEditorMessage() {
  ...
  showSuccess(
    "Message Sent",
    "Your message has been sent to the editor. You can expect a response within 2–5 business days."
  );
}
...
function generateGalley(idx) {
  ...
  const d = new Date();
  doc.text(
    `Published On: ${d.toLocaleDateString("en-GB", {
      day: "2-digit",
      month: "long",
      year: "numeric",
    })}`,
